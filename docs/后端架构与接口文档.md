...
- 障碍记录页：展示持久化障碍边、确认距离和恢复状态，不展示拍照行为及结果。
...
没有记录时返回空数组。没有真实照片时 `image_url` 为 `null`；该字段和 `capture_error` 仅供后端排查，前端不展示。
...
- 首页沿用现有消息列表项，`obstacle` 消息只增加障碍记录入口，不展示缩略图。
- 障碍记录页不展示图片、图片入口或抓拍失败原因。
