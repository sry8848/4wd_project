...
```json
{
  ...
  "eta_text": "来车中",
  "error_message": null,
  "face_verification_id": null,
  "face_verification_image_url": null,
...
| --- | --- |
| `to_pickup` | 小车正在前往起点 |
| `verifying_passenger` | 起点停车，正在核验指定乘客 |
| `waiting_passenger_retry` | 本次核验超时，等待重新识别或取消 |
| `awaiting_boarding_confirmation` | 核验成功，停车等待确认上车 |
| `in_trip` | 已确认上车，正在前往途径点或终点 |
| `classifying_obstacle` | 停车识别障碍颜色，留给后续阶段使用 |
| `scanning_toll_qr` | 停车扫描收费站二维码，留给后续阶段使用 |
| `waiting_toll_clearance` | 停车等待收费站移除，留给后续阶段使用 |
...
创建行程前，前端先调用 `GET /api/passengers`。接口返回后端启动时从 `captures/faces/` 实际加载成功的标签数组；没有有效登记人员时返回 `[]`。
...
1. 校验乘客标签和点位，并拒绝额外字段。
2. 检查导航硬件、固定摄像头和人脸识别器是否可用。
3. 检查是否已有活动行程。
4. 创建行程状态并启动原行程后台线程。
5. 立即返回 `202`，不要让 HTTP 请求阻塞到小车到达终点。

行程执行路径：

```text
当前小车位置 -> 起点停车验脸 -> 等待确认上车 -> 途径点 1 -> ... -> 终点
```
...
- 起点验脸或等待确认期间车辆已经停在可信节点，此时取消直接结束为 `canceled`。
...
记录 ID 不符合固定格式时返回 `400`；记录或真实 JPEG 不存在时返回 `404`。接口必须先校验同名 JSON，禁止把路径片段直接当成文件路径。
