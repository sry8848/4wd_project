...
本文档记录当前 `frontend/` 与树莓派后端的职责、模块边界和 HTTP 接口契约。
...
当前 `frontend/app.js` 使用同源 `/api` 接口创建行程、轮询状态与消息，并读取障碍记录。前端路径函数只用于未提交时预览，真实运行进度以后端为准；QQ 邮件由后端发送，前端不读取邮箱。
...
| `src/server/runtime_state.py` | 保存当前小车状态、活动行程和消息序号 |
...
| `src/server/camera_runtime.py` | 后端进程唯一长期持有 OpenCV 摄像头 |
| `src/server/obstacle_recorder.py` | 在节点恢复和居中结束后组合抓拍与记录保存 |
| `src/server/obstacle_store.py` | 使用同名 JPEG 与 JSON 持久化障碍记录 |
...
| `car` | 小车运行消息 |
| `mail` | 邮件发送结果 |
| `obstacle` | 障碍消息，`obstacle_id` 指向持久化记录 |
...
- `blocked_points` 和 `blocked_edges` 只描述静态地图；单次 `navigate()` 的动态封锁边不写回该接口。
- 已确认动态障碍通过 `GET /api/obstacles` 独立展示。
...
[{
  ...
  "recovered_point": "C3",
  "status": "recovered",
  "image_url": "/api/obstacles/obstacle_20260714_083000_123456_C3_C4/image",
  "capture_error": null
}]
```

没有记录时返回空数组。没有真实照片时 `image_url` 为 `null`，前端显示共享错误图并展示 `capture_error`。

```http
GET /api/obstacles/{record_id}/image
```

该接口只返回已登记且存在的真实 JPEG。JSON 与 JPEG 保存在 `captures/obstacles/`，重启后仍可读取。当前不设数量上限，也不自动删除，因此磁盘占用会持续增长。
...
- 页面进入障碍记录页时调用 `GET /api/obstacles`。
- 首页沿用现有消息列表项，`obstacle` 消息增加缩略图和查看入口。
- 图片失败使用共享的 `frontend/assets/camera-error.svg`，不为每条记录复制默认图。
- QQ 邮件与前端页面无关，由后端到点事件触发。
...
障碍抓拍和图片保存已经接通，但 AI 分析仍是独立的未来能力，不并入当前叫车后端。
...
8. 到达起点、每个途径点和终点时分别异步发送 QQ 邮件。
9. 邮件配置缺失或 SMTP 失败只生成系统消息，不改变行程结果。
10. 障碍恢复居中后保存 JPEG/JSON，并能在后端重启后查询。
11. 任何导航失败都会停车并把行程标记为 `failed`。
...
如果后续需要实时画面，应复用后端持有的同一摄像头会话。运行后端期间不能再启动 `src/tools/stream_camera_mjpeg.py` 抢占设备；该工具只用于后端停止后的独立验证。
