...
- 优先使用 `docs/` 快速定位问题和已有结论，但涉及硬件行为、运行环境、依赖接口等事实时，必须回到厂商原始资料或实机验证。
...
1. 当前树莓派或硬件的实机观测结果，例如通过 SSH 执行只读命令、查看设备节点、版本、日志和真实输出。
2. 与当前硬件型号和开发环境匹配的厂商源代码、硬件手册和厂商教程。
3. 当前项目 `docs/` 中整理过且能追溯到上述来源的结论。
4. 当前仓库代码、参考项目代码和基于经验的推测，只能作为待验证线索，不能单独作为事实依据。

禁止用当前实现反向证明未知细节。当前代码可能建立在未经验证的旧假设上；如果继续依据它推导配置、协议或硬件行为，会形成循环论证并让错误延续。

当资料互相冲突时，不得静默选择：先核对硬件型号、软件版本、GPIO 编号体系和适用环境，再用最小、只读、低风险的 SSH 命令或受控实机实验裁决。无法验证时，必须明确标记“已确认事实”“资料结论”“当前假设”，并向用户说明仍缺少的证据。

## 1.  `docs/`

`docs/` 是当前项目的加工后知识，适合快速定位和决策；它属于二手结论，不能替代厂商原始资料或当前实机证据。
...
- `环境说明/编程教学文档/编程教学文档/`
  ...
  - 用途：查厂商实验说明和教学步骤。
  - 适合问题：第一次验证某个硬件模块时，应该按什么实验顺序测试。或者验证你有没有想错，扩展新思路
