...
import { clearCallLines, drawCallLines } from './call-lines.js';
import { panoramaData } from './panorama-data.js';
import './styles.css';
...
const methodById = new Map(
  panoramaData.methods.map((method) => [method.id, method]),
);
...
function renderCodeTree() {
  const rootDirectory = panoramaData.directories
    .find((directory) => directory.parentId === null);
  const childDirectories = panoramaData.directories
...
function renderStep(method, step) {
  ...
  const calls = panoramaData.calls.filter(
...
function bindMethodInteraction() {
  codeTree.querySelectorAll('[data-method-id]').forEach((details) => {
    details.addEventListener('toggle', requestRedraw);
  });
}
...
function requestRedraw() {
  ...
  redrawFrame = requestAnimationFrame(() => {
    ...
    if (expandedMethodIds.size === 0) {
      clearCallLines(callSvg);
      runtimeErrors.lines = null;
      renderErrors();
      return;
    }

    const calls = panoramaData.calls
      .filter((call) => expandedMethodIds.has(call.sourceMethodId));
    try {
...
function initializePanzoom() {
  ...
  try {
    ...
    controls[2].addEventListener('click', () => panzoomInstance.reset({ animate: true }));
