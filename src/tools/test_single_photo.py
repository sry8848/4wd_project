...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--device-path",
        type=Path,
        help="固定摄像头的稳定 V4L2 路径；传入后不再尝试 --devices。",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def capture_device_with_timeout(
    ...
    参数:
        args: 已解析的命令行参数。
        device_index: OpenCV 摄像头编号或稳定 V4L2 路径。
...
def main() -> int:
    ...
    print(f"尝试摄像头设备: {devices}", flush=True)
    ...
    for device_index in devices:
        ...
        if output_path is None:
            prefix = (
                "single_photo_stable_device"
                if isinstance(device_index, str)
                else f"single_photo_device_{device_index}"
            )
        ...
        if result.device_index is not None:
            print(f"摄像头设备: {result.device_index}", flush=True)
    ...
    print("所有摄像头设备都拍照失败。", file=sys.stderr)
    for error in errors:
        print(f"- {error}", file=sys.stderr)
    print(
        "建议：先确认 USB 连接和稳定设备路径，再检查是否有其它进程占用摄像头。",
        file=sys.stderr,
    )
    return 1
